...
const reviewItems = [
  ...
  },
  {
    ...
    description: "이 회사에서 일하게 될 사람들에게 전하고 싶은 말을 적어주세요.",
    type: "text",
    maxLength: 100,
  },
]
  ...
  const validateCurrentPage = (): string[] => {
    ...
    if (currentPage === 3) {
      reviewItems.forEach((item) => {
        ...
        } else if (item.title === "이 곳에서 일하게 될 사람들에게 한마디") {
          if (!review?.text || review.text.length < 10) {
            errors.push(`${item.title}: 10자 이상 입력해주세요.`)
          }
          if (review?.text && review.text.length > 100) {
            errors.push(`${item.title}: 100자 이하로 입력해주세요.`)
          }
        } else {
          if (!review?.text || review.text.length < 50) {
                ...
                {reviewItems.map((item, index) => (
                  <div key={item.title} className="space-y-3 p-4 border rounded-lg">
                    ...
                    <div>
                      ...
                      <Textarea
                        rows={
                          item.title === "이 리뷰의 한줄 요약" || item.title === "이 곳에서 일하게 될 사람들에게 한마디"
                            ? 2
                            : 4
                        }
                        ...
                        placeholder={
                          item.title === "이 리뷰의 한줄 요약"
                            ...
                            : item.title === "이 곳에서 일하게 될 사람들에게 한마디"
