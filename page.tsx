...
// 리뷰 항목 데이터 (리뷰 제목 문항을 맨 앞에 추가)
const reviewItems = [
  {
    title: "리뷰 제목",
    description: "리뷰 노출시에 보여질 제목을 10자 이상 50자 이내로 적어주세요.",
    type: "title",
  },
  {
    title: "근무환경/시설",
...
export default function DemandReviewForm() {
  ...
  // 현재 페이지 유효성 검사 함수에서 리뷰 제목 검사 로직 추가
  const validateCurrentPage = (): string[] => {
    ...
    if (currentPage === 3) {
      reviewItems.forEach((item) => {
        ...
        if (item.type === "title") {
          if (!review?.text || review.text.length < 10 || review.text.length > 50) {
            errors.push(`${item.title}: 10자 이상 50자 이하로 입력해주세요.`)
          }
  ...
  // handleSubmit 함수에서 리뷰 제목을 Netlify Forms에 전송하도록 추가
              ...
              <div className="space-y-6">
                ...
                {reviewItems.map((item, index) => (
                  <div key={item.title} className="space-y-3 p-4 border rounded-lg">
                    ...
                    <div>
                      <Label className="text-sm">{item.type === "title" ? "제목 *" : "상세 리뷰 *"}</Label>
                      <Textarea
                        rows={item.type === "title" ? 2 : 4}
                        ...
                        placeholder={
                          item.type === "title" ? "10자 이상 50자 이하로 입력해주세요" : "최소 50자 이상 입력해주세요"
                        }
                        className="resize-none"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        {textCounts[item.title] || 0}/{item.type === "title" ? "10-50자" : "50자 이상"}
                      </p>
                    </div>
                  </div>
                ))}
